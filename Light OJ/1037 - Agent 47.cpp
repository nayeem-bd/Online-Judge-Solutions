...
int n,ara[15],target[15][15],dp[1<<15+5];

int solve(int mask)
{
    if(mask == (1<<n)-1)return 0;
    if(dp[mask]!=-1)return dp[mask];
    int cnt=0,tt=0,tmp,i,j,mn = INT_MAX;
    for(i=0;i<n;i++){
        if(check(mask,i)){
            cnt = 0;
            for(j=0;j<n;j++){
                if(!check(mask,j)){
                    cnt = ara[j]/max(1,target[i][j]);
                    if(ara[j]%max(1,target[i][j])){
                        cnt++;
                    }
                    mn = min(mn,cnt+solve(Set(mask,j)));
                }
            }
        }
    }
...
int main()
{
    ...
    */
    TestCase
    {
        ...
        int ans=INT_MAX;
        for(i=0;i<n;i++){
            ans = min(ans,ara[i]+solve(Set(0,i)));
        }
        pfc(cs++),pfi(ans),pn;
    }
